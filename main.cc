...
cNode<int, int>* make_perfect_bst(int n, int offset, int d) {
  ...
  if (right != nullptr) {right->parent = node; right->info.isroot = true; }
  if (left != nullptr) {left->parent = node; left->info.isroot = false; }
  return node;
}
...
void splay_tree_test(int N) {
  ...
  std::cout << "C++ std set insertion took " << elapsed << " seconds" << std::endl;
  start = clock();
  for (int i=0; i < N; i++) {
    T.insert(I[i], I[i]);
  }
  ...
  std::cout << "Splay tree insertion took " << elapsed << " seconds" << std::endl;
  ...
  elapsed = double(clock() - start) / CLOCKS_PER_SEC;
  std::cout << "C++ std set find queries took " << elapsed << " seconds" << std::endl;
  rotations1 = 0;
  start = clock();
  for (int i=0; i < N; i++) {
    int x = rand()%N;
  ...
  elapsed = double(clock() - start) / CLOCKS_PER_SEC;
  std::cout << "Splay tree find queries took " << elapsed << " seconds" << std::endl;
}
